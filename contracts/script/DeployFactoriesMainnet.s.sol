 ...
 *         va compilada como constante en BVCCAgentWalletV4), despues AMBAS
...
contract DeployFactoriesMainnet is Script {
    ...
    bytes32 public constant SALT_SMART    = bytes32(uint256(0x425643430000000A));
    bytes32 public constant SALT_AGENT    = bytes32(uint256(0x425643430000000B));
    ...
    address public constant EXPECTED_SMART_FACTORY = 0xfd105197109244483b5f870501326E6faec9F93c; // V4 — FROZEN build 0.8.36/runs50/cancun
    address public constant EXPECTED_AGENT_FACTORY = 0xf3A61F9d64d45362E149A111289546523BCd26a6; // V4 — FROZEN build 0.8.36/runs50/cancun
    ...
    function predicted() public pure returns (address reg, address smart, address agent) {
        ...
        smart = vm.computeCreate2Address(
            SALT_SMART,
            keccak256(abi.encodePacked(type(BVCCSmartWalletFactoryV4).creationCode, ctorArgs))
        );
        agent = vm.computeCreate2Address(
            SALT_AGENT,
            keccak256(abi.encodePacked(type(BVCCAgentWalletFactoryV4).creationCode, ctorArgs))
        );
    }
    ...
    function run() external {
        ...
        if (EXPECTED_SMART_FACTORY.code.length == 0) {
            new BVCCSmartWalletFactoryV4{salt: SALT_SMART}(FACTORY_OWNER);
            console.log("BVCCSmartWalletFactoryV4 desplegada:", EXPECTED_SMART_FACTORY);
        } else {
            console.log("BVCCSmartWalletFactoryV4 ya existia: ", EXPECTED_SMART_FACTORY);
        }

        if (EXPECTED_AGENT_FACTORY.code.length == 0) {
            new BVCCAgentWalletFactoryV4{salt: SALT_AGENT}(FACTORY_OWNER);
            console.log("BVCCAgentWalletFactoryV4 desplegada:", EXPECTED_AGENT_FACTORY);
        } else {
            console.log("BVCCAgentWalletFactoryV4 ya existia: ", EXPECTED_AGENT_FACTORY);
        }
