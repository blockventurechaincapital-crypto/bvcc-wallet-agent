// Mirrored from docs/contracts.md (monorepo) — keep in sync
import type { DocContent } from '@/components/DocsPage'

export const contracts: DocContent = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Live on Arbitrum One, BNB Chain and Arbitrum Sepolia (testnet). Ethereum and Base deployments are pending and will use the same addresses.',
    },
    ...
    {
      type: 'p',
      text: 'Whitelists max 20 entries each. The four token arrays must have equal length.',
    },
    ...
    { type: 'h3', text: 'Owner functions (callable only via the wallet’s own execute(), i.e. Face ID)' },
    {
      type: 'code',
      lang: 'solidity',
      code: `function authorizeAgent(AuthorizeParams calldata p) external; // re-auth PRESERVES spending history
...
function unpauseAgents() external;`,
    ...
    { type: 'h3', text: 'Agent functions' },
    {
      type: 'code',
      lang: 'solidity',
      code: 'function executeAsAgent(bytes32 mode, bytes calldata executionData) external; // nonReentrant, whenNotPaused',
    },
    {
      type: 'p',
      text: 'See the Agent Integration Guide for call encoding, validation order and error reference.',
    },
    ...
    { type: 'h3', text: 'Getters' },
    {
      type: 'code',
      lang: 'solidity',
      code: `function getAgentPermission(address agent) external view returns (AgentPermission memory);
...
function walletType() public pure returns (uint8); // 0 = personal, 1 = agent`,
    ...
    { type: 'p', text: 'Both factories share the same shape:' },
    {
      type: 'code',
      lang: 'solidity',
      code: `constructor(address owner_); // owner = kill-switch admin, separate from deployer and fee wallet
...
function kill() external; // owner-only, one-way: permanently blocks NEW wallet creation`,
    ...
    },
    {
      type: 'callout',
      tone: 'warn',
      text: 'No external audit yet — this is experimental beta software.',
    },
  ],
}
