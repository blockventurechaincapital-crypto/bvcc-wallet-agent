export const settings = {
  en: {
    settings: {
      ...
      atomicDesc: 'OFF (recommended): dApp batch requests are signed one by one, like a hardware wallet — one Face ID per call, stopping if one fails. ON: the whole batch is signed with a single Face ID and executed atomically (all-or-nothing). Useful for operations that must be indivisible (approve + action in one step, flash loans) and for fewer prompts.',
